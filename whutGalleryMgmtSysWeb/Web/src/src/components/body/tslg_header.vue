  ...
  <div>
    ...
    <div class="search" ref="search" v-if="this.$router.currentRoute.name==='main'">
      <div v-if="!set_top" class="classify" @mouseenter="showClassifyCard" @mouseleave="showClassifyCard"><i></i>分类
        <ul v-if="show_title">
          <li v-for="(item,index) in classify" @click="goBCates(item.name,index)" @mouseenter="showClassifyCard1(index)" @mouseleave="showClassifyCard1(-1)"><span>{{item.name}}</span>
          <ul v-if="index===show_title1">
            <li v-for="(item1,index1) in item.kids" @click.stop="goSCates(item1.name,index,index1)" @mouseenter="showClassifyCard2(index)" @mouseleave="showClassifyCard2(-1)"><span>{{item1.name}}</span>
            <ul v-if="index1===show_title2">
              <li v-for="item2 in item1.kids" @click.stop="goLCates(item2.name,index,index1,index2)"><span>{{item2.name}}</span></li>
            </ul>
            </li>
          </ul>
          </li>
        ...
        <div class="search-top">
          <div class="classify classify1" @mouseenter="showClassifyCard" @mouseleave="showClassifyCard"><i></i>分类
            <ul v-if="show_title">
              <li v-for="(item,index) in classify" @click="goBCates(item.name,index)" @mouseenter="showClassifyCard1(index)" @mouseleave="showClassifyCard1(-1)"><span>{{item.name}}</span>
                <ul v-if="index===show_title1">
                  <li v-for="(item1,index1) in item.kids" @click="goSCates(item1.name,index,index1)" @mouseenter="showClassifyCard2(index)" @mouseleave="showClassifyCard2(-1)"><span>{{item1.name}}</span>
                    <ul v-if="index1===show_title2">
                      <li v-for="item2 in item1.kids" @click="goLCates(item2.name,index,index1,index2)"><span>{{item2.name}}</span></li>
                    </ul>
                  </li>
                </ul>
              </li>
  ...
  export default {
    ...
    mounted(){
      ...
      methods:{
        ...
        goLogin(){
          this.$store.commit('SET_CUR_ROUTER',this.$router.currentRoute.fullPath)
          goLogin()
        },
        goMain(){
